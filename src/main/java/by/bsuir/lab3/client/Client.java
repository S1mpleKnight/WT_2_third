...
import by.bsuir.lab3.connection.Connection;
import by.bsuir.lab3.connection.Message;
import by.bsuir.lab3.connection.MessageType;
import by.bsuir.lab3.scanner.ScanTool;
...
public class Client {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            if (!client.isConnect()) {
                ...
                switch (result) {
                    case 1 -> client.connectToServer();
                    case 2 -> client.finishedProgram();
                }
            }
            if (client.isConnect()) {
                client.logIn();
    ...
    protected void connectToServer() {
        ...
        if (addressServer.equals("-1")) {
            ...
            configureConnection(addressServer);

        }
    }

    private void configureConnection(String addressServer) {
        String[] data = addressServer.split(":");
        String ip = data[0];
        int port = Integer.parseInt(data[1]);
        setUpServerConnection(ip, port);
    }

    private void setUpServerConnection(String ip, int port) {
        if (!isConnect) {
            try {
    ...
    private void processMessageSendRecieve() {
    ...
    protected void makeChoice() {
        showOptionsList();
        int result = getInputOption();
        try {
            switch (result) {
                case 1 -> connection.send(new Message(MessageType.SELECT_ALL_STUDENTS_INFO));
                case 2 -> connection.send(new Message(MessageType.SELECT_STUDENT_INFO_BY_GRADE_BOOK_NUMBER, takeBookNumber()));
                case 3 -> connection.send(new Message(MessageType.SELECT_STUDENTS_INFO_BY_SPECIALITY, takeSpeciality()));
                case 4 -> connection.send(new Message(MessageType.SAVE_STUDENT_INFO, takeStudentInfo()));
                case 5 -> connection.send(new Message(MessageType.UPDATE_STUDENT_INFO_BY_GRADE_BOOK_NUMBER, takeStudentInfoForUpdate()));
                case 6 -> disconnectFromServer();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void showOptionsList() {
        ...
        System.out.println("2)Получить информацию о студенте по номеру студенческого");
        ...
        System.out.println("5)Изменить информацию о студенте по номеру студенческого");
    ...
    protected String takeBookNumber() {
        ...
        while (result.equals("-1")) {
            ...
            if (result.equals("-1")) {
                System.out.println("Неверный номер зачётки. Введите заново");
            }
        }
        return result;
    }
    ...
    protected String takeStudentInfoForUpdate() {
        ...
        while (result.equals("-1")) {
            ...
            if (result.equals("-1")) {
                System.out.println("Неверная информация о студенте. Введите заново");
            }
        }
        return result;
    }
    ...
    protected String takeSpeciality() {
        ...
        while (result.equals("-1")) {
            ...
            if (result.equals("-1")) {
                System.out.println("Неверная специальность. Введите заново");
            }
        }
        return result;
    }
